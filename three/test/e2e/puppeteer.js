	...
	for ( let id = beginId; id < endId; ++ id ) {
		...
		while ( attemptId < maxAttemptId ) {
			...
			if ( process.env.MAKE ) {
				...
				await page.screenshot( { path: `./examples/screenshots/${ file }.webp` } );
				printImage( png.sync.read( fs.readFileSync( `./examples/screenshots/${ file }.webp` ) ), console );
				console.green( `file: ${ file } generated` );


			} else if ( fs.existsSync( `./examples/screenshots/${ file }.webp` ) ) {


				/* Diff screenshots */

				let actual = png.sync.read( await page.screenshot() );
				let expected = png.sync.read( fs.readFileSync( `./examples/screenshots/${ file }.webp` ) );
