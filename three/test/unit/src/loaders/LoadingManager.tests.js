...
export default QUnit.module( 'Loaders', () => {
	...
	QUnit.module( 'LoadingManager', () => {
		...
		QUnit.test( "getHandler", ( assert ) => {
			...
			const regex1 = /\.webp$/i;
			const regex2 = /\.webp$/gi;

			loadingManager.addHandler( regex1, loader );

			assert.equal( loadingManager.getHandler( 'foo.webp' ), loader, 'Returns the expected loader.' );
			assert.equal( loadingManager.getHandler( 'foo.webp.webp' ), null, 'Returns null since the correct file extension is not at the end of the file name.' );
			...
			assert.equal( loadingManager.getHandler( 'foo.webp' ), loader, 'Returns the expected loader when using a regex with "g" flag.' );
			assert.equal( loadingManager.getHandler( 'foo.webp' ), loader, 'Returns the expected loader when using a regex with "g" flag. Test twice, see #17920.' );
